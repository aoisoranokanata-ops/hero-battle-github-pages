...
(function(root){root.Session={
 ...
 undo(s){if(!s.history.length)return s;const history=[...s.history],game=Game.lock(history.pop());game.timer.warningFlags={...game.timer.warningFlags,...s.game.timer.warningFlags};game.timer.lastChecked=Game.remaining(s.game);if(game.timer.running)game.timer.at=Date.now();return {game,history,transactionOpen:false};}
};})(globalThis);
