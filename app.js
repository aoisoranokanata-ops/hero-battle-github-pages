...
(() => {
  ...
  Presentation.configure(()=>rules);
  function run(action){
    try {const before=game,visual=Presentation.capture(game);const next=Session.perform({game,history,transactionOpen},action);({game,history,transactionOpen}=next);choiceIds=[];persist();render();Presentation.transition(before,game,action,visual);return true;}catch(e){toast(e.message);return false;}
  }
  function navigate(next){Presentation.clear();closeModal();if(screen==='battle'&&next!=='battle'&&game){game=Game.lock(game);persist();}screen=next;render();window.scrollTo(0,0);}
  function home(){
  ...
  function modifierBadges(c){return (c.modifiers||[]).filter(m=>m.until).map(m=>`<small class="temporary-badge">${esc(Object.entries(m.stats||{}).map(([k,v])=>({attack:'攻撃',defense:'防御',maxTroop:'最大兵力'}[k])+' '+(v>=0?'+':'')+v).join(' / ')||(m.extraAttacks?'追加攻撃権 '+m.extraAttacks+'回':'一時状態'))}<br>${esc(Game.durationName(m))}</small>`).join('');}
  ...
  document.addEventListener('click',e=>{
    ...
    try {switch(act){
      ...
      case 'createMatch':{
        ...
        game=Game.create(ds,rules,minutes*60,Number($('#first').value));history=[];transactionOpen=false;persist();navigate('battle');Presentation.start();break;
      }
      ...
      case 'undo':Presentation.clear();if(history.length&&confirm('関連する処理をまとめて1つ戻しますか？')){({game,history,transactionOpen}=Session.undo({game,history,transactionOpen}));closeModal();persist();render();}break;
  ...
  document.addEventListener('visibilitychange',()=>{if(document.hidden)Presentation.clear();if(game&&document.hidden){game=Game.lock(game);closeModal();render();persist();}});
