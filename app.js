...
(() => {
  ...
  function run(action){
    try {const next=Session.perform({game,history,transactionOpen},action);({game,history,transactionOpen}=next);choiceIds=[];persist();render();return true;}catch(e){toast(e.message);return false;}
  }
  function navigate(next){closeModal();if(screen==='battle'&&next!=='battle'&&game){game=Game.lock(game);persist();}screen=next;render();window.scrollTo(0,0);}
  function home(){
    return `${Store.read('match',null)?.game?'<p><a href="legacy/index.html">保存済みPhase 1の試合を旧ルールで続ける</a></p>':''}${game?`<div class="resume row"><div><strong>前回の試合が保存されています</strong><br><small>${esc(game.players[0].name)} 対 ${esc(game.players[1].name)} · ${game.turn}ターン${game.result?' · 終了':''}</small></div>${btn('前回の試合を再開','resume','','primary')}</div>`:''}
      ...
      <section class="steps"><div><span class="step-number">01 / デッキを用意</span><h3>108枚から、10枚を選ぼう</h3><p>レアリティの枚数を確認しながら、自分だけのデッキを保存。</p></div><div><span class="step-number">02 / 自動で手札を配布</span><h3>手札から場に出そう</h3><p>引く操作は不要。場に出す時に実物の紙カードを並べます。</p></div><div><span class="step-number">03 / 相談しながら対戦</span><h3>スキルを読んで、作戦を立てよう</h3><p>効果の補正も、操作を戻すことも。2人で1台を使えます。</p></div></section><p class="muted">収録：UR 9枚 ／ SSR 27枚 ／ SR 27枚 ／ R 45枚</p>`;
  }
  ...
  function fieldCard(c,p){
    ...
    return `<article class="card ${raw.rarity} ${c.troop===0?'zero':''}"><span class="badge">${c.id} · ${raw.rarity}</span><h3>${esc(raw.name)}</h3>${Game.automationType(c.id)==='MANUAL'?btn('⚠ 手動処理','manualInfo',`data-player="${p}" data-id="${c.id}"`,'manual-badge'):''}${Game.abilities(game,p,c).map(a=>a.available?btn(a.trigger==='passive'?'スキル発動！【継続効果】':'スキル発動！','ability',`data-player="${p}" data-id="${c.id}" data-slot="${a.slot}"`,'skill-badge'):a.used?'<span class="state">'+esc(a.sourceName)+'：発動済み</span>':'').join('')}<small>コスト ${raw.cost}</small><div class="stats"><span>攻 <strong>${c.attack}</strong></span><span>防 <strong>${c.defense}</strong></span></div><div class="troop">${c.troop} / ${c.maxTroop}<small> 兵力</small></div><meter min="0" max="${c.maxTroop}" value="${c.troop}" aria-label="${esc(raw.name)}の兵力"></meter><div>${stateLabels(c)}</div>${c.note?`<p class="notice">${esc(c.note)}</p>`:''}<div class="card-actions">${btn('スキル・特性','detail',`data-player="${p}" data-id="${c.id}"`)}${btn('効果','effects',`data-player="${p}" data-id="${c.id}" ${game.result?'disabled':''}`)}</div><div class="card-actions">${btn('攻撃','attackSelect',`data-player="${p}" data-id="${c.id}" ${attack?'':'disabled'}`,'primary')}${c.troop===0?btn('トラッシュ','trashConfirm',`data-player="${p}" data-id="${c.id}"`,'danger'):''}</div></article>`;
  }
  function playerPanel(p,i){
    const ended=!!game.result, secret=i!==game.viewer||p.hidden;
    return `<section class="player ${i===game.active?'active':''} ${i?'player-b':''}"><div class="identity"><span class="badge player-mark">プレイヤー${i?'B':'A'}${i===game.active?' · あなたのターン':''}</span><h2>${esc(p.name)}</h2><small>${esc(p.team)}${p.team?' ／ ':''}${esc(p.deckName)}</small></div><div class="counters"><div class="counter life"><div>ライフ</div><div class="big">${p.life}</div>${btn('−','life',`data-player="${i}" data-delta="-1" aria-label="${esc(p.name)}のライフを1減らす" ${ended?'disabled':''}`)} ${btn('＋','life',`data-player="${i}" data-delta="1" aria-label="${esc(p.name)}のライフを1増やす" ${ended?'disabled':''}`)} ${btn('調整','lifePanel',`data-player="${i}" ${ended?'disabled':''}`)}</div><div class="counter"><div>エネルギー</div><div class="big">${p.energy}<small> / ${p.maxEnergy}</small></div>${btn('−','energy',`data-player="${i}" data-delta="-1" aria-label="${esc(p.name)}のエネルギーを1減らす" ${ended?'disabled':''}`)} ${btn('＋','energy',`data-player="${i}" data-delta="1" aria-label="${esc(p.name)}のエネルギーを1増やす" ${ended?'disabled':''}`)}</div></div><div class="zones">${btn(`山札 ${p.deck.length}枚`,'zone',`data-player="${i}" data-zone="deck"`)}${btn(`トラッシュ ${p.trash.length}枚`,'zone',`data-player="${i}" data-zone="trash"`)}${i===game.viewer?btn('追加ドロー（手動）','drawPanel',`data-player="${i}" ${ended?'disabled':''}`):''}</div><div class="row"><h3>場 <small>${p.field.length} / ${game.rules.fieldLimit}</small></h3><small>攻撃・防御・兵力</small></div><div class="field-grid">${p.field.map(c=>fieldCard(c,i)).join('')}${Array.from({length:Math.max(0,game.rules.fieldLimit-p.field.length)},()=>'<div class="empty-slot">場の空き</div>').join('')}</div><div class="divider"></div><div class="row"><h3>手札 ${p.hand.length}枚</h3>${i===game.viewer?btn(p.hidden?'手札を表示':'手札を隠す','hide',`data-player="${i}" ${ended?'disabled':''}`):''}</div><div class="hand">${secret?'<p class="card-backs" aria-label="非公開の手札'+p.hand.length+'枚">'+'■'.repeat(p.hand.length)+'</p>':p.hand.length?p.hand.map(id=>btn(`<small>${id} · コスト ${Game.card(id).cost}</small>${esc(Game.card(id).name)}`,'handCard',`data-player="${i}" data-id="${id}"`)).join(''):'<p class="muted">手札はありません</p>'}</div></section>`;
  }
  function battle(){
  ...
  function manualInfo(p,id){const texts=['skill','trait'].map(slot=>Game.metadata(id,slot)).filter(a=>a.automationType==='MANUAL');showModal('⚠ 手動処理',texts.map(a=>`<h3>${esc(a.sourceName)}</h3><p class="detail-text">${esc(a.sourceText)}</p><p>${esc(a.reason)}</p>`).join('')+btn('効果調整','effects',`data-player="${p}" data-id="${id}"`));}
  ...
  document.addEventListener('click',e=>{
    ...
    try {switch(act){
      ...
      case 'createMatch':{
        ...
        game=Game.create(ds,rules,minutes*60,Number($('#first').value));history=[];transactionOpen=false;persist();navigate('battle');break;
      }
      case 'resume':if(saved?.source&&saved.source!==CARD_SOURCE.sha256)toast('カード原本が更新されています。対戦中の数値は保存値を使用します。');navigate('battle');break;
      case 'ready':closeModal();run({type:'ready'});break;
      ...
      case 'ability':{const a=Game.metadata(id,d.slot);if(a.trigger==='passive')detail(id,p);else if(game.pending.length)render();else run({type:'activate',player:p,id,slot:d.slot});break;}
      case 'undo':if(history.length&&confirm('関連する処理をまとめて1つ戻しますか？')){({game,history,transactionOpen}=Session.undo({game,history,transactionOpen}));closeModal();persist();render();}break;
      ...
      case 'adjust':if(run({type:'adjust',player:p,id,key:d.key,delta,reset:d.reset==='true'},false)){const c=game.players[p].field.find(c=>c.id===id);if(!c||game.handoff||game.result)closeModal();else effects(p,id);}break;
  ...
  document.addEventListener('visibilitychange',()=>{if(game&&document.hidden){game=Game.lock(game);closeModal();render();persist();}});
